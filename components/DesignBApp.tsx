  ...
  return (
    <View style={{ flex: 1, backgroundColor: C.bg }}>
      <View style={{ position:'absolute', right:-30, bottom:100 }} pointerEvents="none">
        <Image source={require('../assets/images/design-b/logo-hex.png')}
          style={{ width:260, height:260, opacity:0.08 }}
          resizeMode="contain" />
      </View>
...
function ReceiptScreen({
  ...
  const handleSharePDF = async () => {
    ...
    setBusy(true);
    try {
      const html = makeReceiptHTML(data.type, data.amount, data.status, data.details);
  ...
  const handleShareImage = () => {
    if (imgBusy) return;
    setImgBusy(true);
    setImgHTML(buildReceiptCanvasHTML(data.type, data.amount, data.status, data.details));
  };
              ...
              <View style={{ flexDirection: 'row', gap: 12, marginBottom: 12 }}>
                <Pressable onPress={handleShareImage} disabled={imgBusy}
                  style={({ pressed }) => [{
                    ...
                    opacity: (pressed || imgBusy) ? 0.55 : 1,
                  }]}>
                  <Ionicons name="image-outline" size={16} color={C.primary} />
                  <Text style={{ fontFamily: C.bold, fontSize: 13, color: C.primary }}>{imgBusy ? 'Generating…' : 'Share as Image'}</Text>
                </Pressable>
                <Pressable onPress={handleSharePDF} disabled={busy}
                  ...
                  <Text style={{ fontFamily: C.bold, fontSize: 13, color: C.primary }}>{busy ? 'Generating…' : 'Share as PDF'}</Text>
                </Pressable>
              </View>
